...
async def startupmessage():
    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await jepiq.tgbot.send_file(
                ...
                caption="**᯽︙ بــوت سورس العرب يـعـمـل بـنـجـاح ✓ **\n**᯽︙ ارسل `.الاوامر` لرؤية اوامر السورس**",
                buttons=[(Button.url("كروب سورس العرب", "https://t.me/dfj5_4"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
    ...
    try:
        if msg_details:
            ...
            text = (
                message.text
                + "\n\n**᯽︙اهلا وسهلا لقد قمت باعاده تشغيل بـوت سورس العرب تمت بنجاح**"
            )
...
async def add_bot_to_logger_group(chat_id):
    ...
    try:
        ...
        try:
            ...
            LOGS.error(str(e))
#by @dfj5_4 بس اشوفك خامطه للكود اهينك وافضحك 
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        descript = "- عزيزي المستخدم هذه هي مجموعه الاشعارات يرجى عدم حذفها  - @dfj5_4"
        photobt = await jepiq.upload_file(file="JepIQ/razan/resources/start/Jepthon.jpg")
        _, groupid = await create_supergroup(
            "مجموعة اشعارات سورس العرب ", jepiq, Config.TG_BOT_USERNAME, descript, photobt
        )
